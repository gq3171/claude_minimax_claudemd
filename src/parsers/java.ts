...
export class JavaParser {
    ...
    for (const child of paramsNode.children) {
      if (child.type === "formal_parameter") {
        ...
        if (nameNode) {
          ...
          const paramType = typeNode ? sourceCode.substring(typeNode.startIndex, typeNode.endIndex) : "Object";
  ...
  private hasPlaceholder(bodyNode: Parser.SyntaxNode | null, sourceCode: string): boolean {
    ...
    return /throw new UnsupportedOperationException|throw new RuntimeException\("not implemented"\)/.test(bodyText);
  }
}
